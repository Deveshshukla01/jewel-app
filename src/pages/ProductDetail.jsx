...
//               <h3 className="text-xl font-semibold text-primary-800 mb-4">Product Description</h3>
...
//                   <span className="w-2 h-2 bg-secondary-600 rounded-full mr-3 animate-pulse"></span>
...
//                 className="w-full bg-gradient-to-r from-green-500 to-green-600 text-white py-4 px-8 rounded-2xl font-semibold text-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105 flex items-center justify-center space-x-3 glow"
//               >
    ...
    return (
      <div className="min-h-screen bg-white">
        ...
        <div className="container mx-auto px-4 py-16 text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-4 border-amber-200 border-t-amber-700 mx-auto"></div>
          <p className="mt-6 text-brown-700 text-lg">Loading product details...</p>
        </div>
        <Footer />
      </div>
    )
  ...
  if (error || !product) {
    return (
      <div className="min-h-screen bg-amber-50">
        ...
        <div className="container mx-auto px-4 py-16 text-center">
          <div className="text-4xl mb-4">⚠️</div>
          <p className="text-lg mb-4 text-brown-700">{error || 'Product not found'}</p>
          <Link to="/" className="text-yellow-700 hover:text-yellow-900 font-medium underline">
            Return to Home
          </Link>
        </div>
        <Footer />
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="container mx-auto max-w-7xl px-4 py-10">
        {/* Breadcrumb */}
        <nav className="flex items-center space-x-2 text-sm text-brown-600 mb-6">
          <Link to="/" className="hover:text-yellow-700">Home</Link>
          <span>/</span>
          <Link to={`/category/${product.category}`} className="hover:text-yellow-700">{categoryName}</Link>
          <span>/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
          ...
          <div className="relative rounded-xl overflow-hidden">
            <img
              src={product.imageUrl || 'https://images.pexels.com/photos/1191531/pexels-photo-1191531.jpeg?auto=compress&cs=tinysrgb&w=800'}
              alt={product.name}
              className="w-120 h-80 md:h-[400px] object-cover rounded-xl"
            />
            {product.featured && (
              <span className="absolute top-4 left-4 bg-yellow-700 text-white px-3 py-1 rounded-md text-sm">
                Featured
              </span>
            )}
          </div>
          ...
          <div className="space-y-6">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold text-brown-900 font-serif">
                {product.name}
              </h1>
              <p className="mt-2 text-sm text-brown-600">{categoryName}</p>
              <div className="text-3xl font-bold text-yellow-800 mt-4">
                ₹{product.price?.toLocaleString()}
              </div>
            </div>
            ...
            <p className="text-brown-700 leading-relaxed">
              {product.description || `Exquisite ${product.name} crafted with elegance and precision. A timeless piece from our ${categoryName.toLowerCase()} collection.`}
            </p>

            {/* Features (inline list instead of box) */}
            ...
            <button
              onClick={handleWhatsAppContact}
              className="w-full bg-green-600 hover:bg-green-700 text-white py-3 px-6 rounded-lg font-medium text-lg transition"
            >
