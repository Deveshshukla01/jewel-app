      ...
      <div className="relative max-w-7xl mx-auto px-4 font-serif">
        ...
        <div className="relative">
          <AnimatePresence mode="wait">
            <motion.div
              key={index} // re-trigger animation when index changes
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              transition={{ duration: 0.6, ease: "easeInOut" }}
              className={`grid gap-8 sm:gap-10 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 justify-items-center`}
            >
              {data.slice(index, index + itemsPerPage).map((r, i) => (
                <motion.div
                  key={i}
                  className="bg-white p-6 sm:p-8 rounded-2xl shadow-md border border-gray-200 w-80 sm:w-[20rem] flex flex-col items-center text-center justify-between hover:shadow-xl transition-all"
